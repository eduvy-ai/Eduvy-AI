...
// CRUD interface for education boards

import React, { useEffect, useState, useMemo } from 'react'
import { useBoards, useCanEdit } from '../../hooks'
...
const BoardsPage: React.FC = () => {
  const { boards, fetchBoards, addBoard, updateBoard, removeBoard } = useBoards()
  ...
  const handleCreate = () => {
    ...
    setFormData({ id: '', name: '', sort_order: boards.length, is_active: true })
    setFormError('')
    setShowModal(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    if (!formData.id.trim()) {
      setFormError('Board ID is required')
      return
    }
    ...
    if (!/^[a-z0-9_]+$/.test(formData.id)) {
      setFormError('Board ID must be lowercase alphanumeric with underscores')
      return
    }

    try {
      if (editingBoard) {
        const updated = await adminApi.boards.update(editingBoard.id, {
          name: formData.name,
        ...
        const created = await adminApi.boards.create({
          id: formData.id,
          name: formData.name,
          sort_order: formData.sort_order,
          is_active: formData.is_active,
      ...
      setFormError(error.response?.data?.detail || 'Failed to save board')
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      await adminApi.boards.delete(deleteTarget.id)
      removeBoard(deleteTarget.id)
      setShowDeleteConfirm(false)
      setDeleteTarget(null)
    } catch (error: any) {
      setFormError(error.response?.data?.detail || 'Failed to delete board')
    }
  }
  ...
  const handleImport = async () => {
    try {
      ...
      await adminApi.boards.bulkImport(boards)
      await fetchBoards()
  ...
  const toggleSelectAll = () => {
    const pageIds = paginatedBoards.map(b => b.id)
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await adminApi.boards.bulkDelete(Array.from(selectedIds))
      selectedIds.forEach(id => removeBoard(id))
      setSelectedIds(new Set())
      setShowDeleteConfirm(false)
      setDeleteTarget(null)
    } catch (error: any) {
      setFormError(error.response?.data?.detail || 'Failed to delete boards')
    }
  }
  ...
  const columns: TableColumn<Board>[] = [
    ...(canEdit ? [{
      key: 'select' as keyof Board,
      header: (
        <input
          type="checkbox"
          checked={paginatedBoards.length > 0 && paginatedBoards.every(b => selectedIds.has(b.id))}
    ...
    { key: 'sort_order', header: 'Order', width: '80px' },
    {
      ...
      width: '100px',
      render: (board) => (
        <span className={`inline-flex items-center gap-1 text-xs font-medium ${board.is_active ? 'text-app-green' : 'text-app-muted'}`}>
          {board.is_active ? <CheckCircle size={14} weight="fill" /> : <XCircle size={14} />}
          {board.is_active ? 'Active' : 'Inactive'}
        </span>
      ),
    },
    ...(canEdit ? [{
      key: 'actions' as keyof Board,
      header: 'Actions',
      width: '120px',
      render: (board: Board) => (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-app-text">Boards</h1>
          <p className="text-sm text-app-muted mt-1">Manage education boards</p>
        </div>
        {canEdit && (
          <div className="flex items-center gap-2">
            {selectedIds.size > 0 && (
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-app-muted mb-1.5">Board ID</label>
            <input
              type="text"
              value={formData.id}
          ...
          <div>
            <label className="block text-sm font-medium text-app-muted mb-1.5">Board Name</label>
            <input
              type="text"
              value={formData.name}
      ...
      <Modal
        ...
        title={deleteTarget ? 'Delete Board' : `Delete ${selectedIds.size} Board${selectedIds.size > 1 ? 's' : ''}`}
        size="sm"
      >
        <div className="space-y-4">
          {deleteTarget ? (
            <p className="text-app-text">
              Are you sure you want to delete <strong>{deleteTarget.name}</strong>? This will also remove all curriculum entries for this board.
            </p>
          ) : (
            <p className="text-app-text">
              Are you sure you want to delete <strong>{selectedIds.size} board{selectedIds.size > 1 ? 's' : ''}</strong>? This will also remove all related curriculum entries.
            </p>
          )}
          {formError && (
            <div className="p-3 rounded-xl bg-app-red/10 border border-app-red/30 text-sm text-app-red">
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-app-muted">
            Paste JSON array of boards. Each board should have: id, name, sort_order, is_active
          </p>
          <textarea
            value={importJson}
            onChange={(e) => setImportJson(e.target.value)}
            placeholder={`[
  { "id": "cbse", "name": "CBSE", "sort_order": 0, "is_active": true },
  { "id": "icse", "name": "ICSE", "sort_order": 1, "is_active": true }
]`}
