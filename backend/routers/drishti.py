"""
drishti.py — Drishti Vision-Accessible Learning helper portal API.

Authentication: X-Helper-Token header (UUID issued at helper creation — no password needed).

GET    /api/helper/me                      → verify token, return helper info
GET    /api/helper/students                → assigned students with XP, streak, last active
POST   /api/helper/notes                   → send encouragement note to a student
GET    /api/helper/notes/{student_id}      → note history for a specific student

Student-facing (auth via Bearer token):
GET    /api/profile/{id}/helper-notes      → unread notes for this student
POST   /api/profile/{id}/helper-notes/read → mark all notes read
"""
...
# ── Student-facing note endpoints (Bearer auth) ────────────────────
